...
def main():
    ...
    parser.add_argument("--use-pi-low", action="store_true", help="Also compute conservative status from PI low bound")
    ...
    # conservative PI-based status
        # conservative PI-based status + SAFE gating policy
    if args.use_pi_low:
        ...
        # 1) Pure conservative (status only from PI low)
        ...
        # 2) SAFE gated: keep point status, but never allow GREEN if PI low indicates risk
        def safe_gate(point_status: str, pi_low_val: float) -> str:
            # If point says GREEN, but PI-low crosses thresholds -> downgrade
            if point_status == "GREEN":
                if pi_low_val <= args.red_thr:
                    return "RED"
                if pi_low_val <= args.yellow_thr:
                    return "YELLOW"
                return "GREEN"

            # Optional: escalate YELLOW->RED if PI-low is RED
            if point_status == "YELLOW" and pi_low_val <= args.red_thr:
                return "RED"
